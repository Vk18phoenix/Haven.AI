...
// --- THIS IS THE CORRECT CONFIGURATION FOR YOUR USE CASE ---
const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
const recognition = SpeechRecognition ? new SpeechRecognition() : null;
if (recognition) {
  recognition.continuous = false; // Stop listening after a pause in speech
  recognition.lang = 'en-US';
  recognition.interimResults = false; // We only want the final, complete result
}
...
const MainLayout = ({ user }) => {
  ...
  // It can now be called with text directly from the speech recognition
  ...
  // --- SET UP SPEECH RECOGNITION EVENTS ONCE ---
  useEffect(() => {
    if (!recognition) return;

    // SUCCESS: When the user stops talking, this event fires
    recognition.onresult = (event) => {
      const transcript = event.results[0][0].transcript;
      handleSendMessage(null, transcript); // Automatically send the final transcript
    };
    
    // FAILURE: If the mic hears nothing or there's an error
    recognition.onerror = (event) => {
      if (event.error === 'no-speech' || event.error === 'network') {
        ...
        setMessages(prev => [...prev, errorMessage]); // Add the error message directly to the chat
      } else {
        toast.error(`Speech recognition error: ${event.error}`);
      }
    };

    // ALWAYS fires when recognition ends
    recognition.onend = () => {
  ...
  }, [messages, activeChatId, chatHistory]); // Re-attach listeners if chat context changes
  ...
  const speakText = (text) => {
    if (!aiVoice || !text || window.speechSynthesis.speaking) return;
    const utterance = new SpeechSynthesisUtterance(text);
    const voices = window.speechSynthesis.getVoices();
    let selectedVoice = null;
    if (aiVoice === 'female') {
      selectedVoice = voices.find(v => v.name.includes('Google US English') && v.name.includes('Female')) || voices.find(v => v.lang.startsWith('en') && (v.name.includes('Female') || v.name.includes('Zira') || v.name.includes('Susan')));
    } else {
      selectedVoice = voices.find(v => v.name.includes('Google US English') && !v.name.includes('Female')) || voices.find(v => v.lang.startsWith('en') && (v.name.includes('Male') || v.name.includes('David')));
    }
  ...
  const handlePin = async (chatId) => {
    ...
    setChatHistory(updatedHistory); await saveUserChatHistory(user.uid, updatedHistory); toast.success(chatToPin.pinned ? 'Chat unpinned!' : 'Chat pinned!');
  };
  const handleRename = async (chatId, newTitle) => {
    ...
    setChatHistory(updatedHistory); setRenamingChatId(null); await saveUserChatHistory(user.uid, updatedHistory); toast.success('Chat renamed!');
  };
  const handleDelete = async (chatId) => {
    const updatedHistory = chatHistory.filter(chat => chat.id !== chatId);
    setChatHistory(updatedHistory); if (activeChatId === chatId) handleNewChat(); await saveUserChatHistory(user.uid, updatedHistory); toast.success('Chat deleted!');
  };
  const handleShare = (chatId) => {
    const chat = chatHistory.find(c => c.id === chatId); const shareText = `Check out my conversation with Haven.AI: "${chat.title}"`;
    if (navigator.share) {
      navigator.share({ title: 'Haven.AI Chat', text: shareText, url: window.location.href });
    } else {
      navigator.clipboard.writeText(`${shareText} - ${window.location.href}`); toast.success('Link copied to clipboard!');
    }
  };
      ...
      {contextMenu.visible && (() => {
          ...
          return (
            <div className="context-menu" style={{ top: contextMenu.y, left: contextMenu.x }}>
              <div onClick={() => handlePin(contextMenu.chatId)}> {chat?.pinned ? <PinOff size={16}/> : <Pin size={16}/>} {chat?.pinned ? 'Unpin' : 'Pin'} </div>
              <div onClick={() => setRenamingChatId(contextMenu.chatId)}><Edit size={16}/> Rename</div>
              <div onClick={() => handleShare(contextMenu.chatId)}><Share2 size={16}/> Share</div>
              <div className="delete" onClick={() => handleDelete(contextMenu.chatId)}><Trash2 size={16}/> Delete</div>
            </div>
          );
      })()}
        ...
        <div className="sidebar-content">
          ...
          {chatHistory.sort((a, b) => (b.pinned || 0) - (a.pinned || 0) || b.id - a.id).map(chat => (
            renamingChatId === chat.id ? (
              ...
              <div key={chat.id} className={`chat-history-item ${chat.id === activeChatId ? 'active' : ''}`} onClick={() => handleSelectChat(chat.id)} onContextMenu={(e) => handleContextMenu(e, chat.id)}>
                {chat.pinned && <Pin size={14} className="pin-icon" />} {chat.title}
              </div>
            )
          ))}
        </div>
